...
export default function App() {
  ...
  const [vtela, setVtela] = useState(estados.valorTelas)
  ...
  const addDigito = (d) => {
    ...
    if((d=='+' || d=='-' || d=='*' || d=='/') && estados.operado){
      estados.valorTelas=estados.resultado
      estados.resultado=0
    }
    estados.valorTelas = estados.valorTelas + d
    setVtela(estados.valorTelas)
  ...
  const opera=(d)=>{
    ...
    if(d=='BS'){
      estados.valorTelas=vtela.substring(0,(vtela.length-1))
      setVtela(estados.valorTelas)
      return
    }
    try{
      estados.resultado=eval(estados.valorTelas)
  ...
  return (
    <SafeAreaView style={estilos.conteiner}>
      ...
      <View style={estilos.botoes}>
        ...
        <Btn label="+" operacao aoClicar={() => { addDigito('+') }} ></Btn>
        <Btn label="0" operacao aoClicar={() => { addDigito('0') }} ></Btn>
        <Btn label="." aoClicar={() => { addDigito('.') }} ></Btn>
